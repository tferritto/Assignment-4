...
void show_list(sequenceDouble src);
// Pre: (none)
// Post: The items of src are printed to cout (one per line).
void show_list(sequenceChar src);
// Pre: (none)
// Post: The items of src are printed to cout (one per line).
int get_object_num();
// Pre:  (none)
...
myT get_item(myT theObj);
// Pre:  (none)
// Post: The user is prompted to enter a real number or character. The prompt
//       is repeated until a valid real number or character can be read. The
//       valid real number or character read is returned. The input buffer is
//       cleared of any extra input until and including the
//       first newline character.

int main(int argc, char *argv[])
{
    sequenceDouble s1;  // A sequence of double for testing
    sequenceChar s2; // A sequence of char for testing
    ...
    do
    {
        ...
        switch (choice)
        {
            ...
            case 'A':
                ...
                if (objectNum == 1)
                {
                    numHold = get_item<double>(objectNum);
                    s1.add(numHold);
                    cout << numHold << " added to s1." << endl;
                }
                else
                {
                    charHold = get_item<char>(objectNum);
                    s2.add(charHold);
...
myT get_item(myT theObj)
{
    ...
    // request real number if object is S1
    if(theObj == 1){
	...
	// request character if object is S2
	if(theObj == 2){
